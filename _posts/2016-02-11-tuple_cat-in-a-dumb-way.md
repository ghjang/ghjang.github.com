...
다소 복잡한 C++ TMP를 이용해서 효율적인 `tuple_cat`을 구현하는 것과 관해서는 아래의 글들을 참고할만하다:
...
`tuple_cat_` 템플릿함수내에서 `std::tuple` 객체에 대해서 오버로딩한 `operator +` 함수를 C++17 fold 표현식내에서 찾을 수 없다는 내용이다. `operator +`를 `std::tuple` 선언전에 놓아야한다는 것 같은데, 현재의 이 구조로는 불가능한 일이다. C++17 fold 표현식에서는 C++ 자체의 연산자를 이용한 표현식만을 사용할 수 있다는 것 같다. `std::tuple` 자체에 `tuple_cat` 용도의 연산자 오버로딩을 **새로이** 추가할 수는 없다는 상황인 것이다. `std::tuple`에 대한 `operator +` 오버로딩이 원래 표준에 있었다면 **템플릿 특수화**의 형태로 추가가 가능은 했을 것이다. 이 것에 대한 우회책으로 `std::tuple` 인자를 wrapping해주는 것을 간단히 생각해 볼 수 있을 것이다. 대략 이런 식이라는 것이다.
...
동작을 하기는 하지만, 언뜻봐도 문제점들이 곧바로 보이게된다:

1. 전형적으로 알고있던 fold부류의 함수의 모양새가 아니어서 처음에는 좀 불편할 수 있겠다.
2. value category에 대한 고려가 없어 성능개선이 필요하다. `tuple_cat_`에 rvalue인자만 넘어오면 문제없지만, lvalue인자가 있을 경우 복사가 발생하게된다.

1번 문제의 경우는 구현을 조금만 손보면 해결볼 수 있겠다.
...
변경된 `ArgWrap` 내부에서 `operator +`를 rvalue레퍼런스에 대해서만 오버로딩한 것은 `ArgWrap`은 세부구현사항이고 lvalue객체로 사용될 경우가 없기 때문이다.

`ArgWrap` 특수화내 일종의 **tag dispatch pattern** 형태의 오버로딩된 코드는 C++17에 추가된다는 `if constexpr` 같은 코어언어기능을 이용하면 좀더 간소화할 수는 있겠다.

최종적인 코드를 테스트를 많이 한 것은 아니라 구현이 정확하지는 않을 수도 있겠다. **연습**을 목표로 한 것이니 이정도에서 만족하고 접도록 한다.

개인적으로 **fold적인 사고방식**을 가지려고 노력중이다. 재미진다. **Functional Programming**에서 fold를 가리켜서 **Super High-Order Function**이라고 하는 경우도 봤다. 그만큼 매우 빈번히 사용되고 강력하다는 것이다. 많은 알고리즘이 최종적으로 fold로 귀결되는 모습을 보고 있자면 가끔 경이롭기까지한 느낌이랄까나.
...
+ [Fold it the way you like]({{ site.baseurl }}{% post_url 2015-12-12-about-fold %}): C++17 fold에 관한 내용은 아님. C++ TMP 공부하는 와중에 정리해본 내용임.
