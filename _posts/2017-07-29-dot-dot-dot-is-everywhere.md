...
`...` 표기를 통한 컴파일 타임 가변 템플릿 인수를 다루는 표현의 도입으로 C++11 이전에는 매우 불편하고 비효율적이며 지저분한 코드가 단순해졌다. 코드의 표현력 또한 향상시켰다. 여기에서 다루지못한 `...` 표기 사용 컨텍스트도 있겠다. 또한 다음 표준에서도 계속 언어 스펙이 확장되기에 좀더 사용 범위가 늘어날 듯하다.
